...
from app.models import NewsletterPick
...
def test_newsletter_pro_gets_full_unlimited_research() -> None:
    token, _ = _register()
    headers = {"Authorization": f"Bearer {token}"}
    upgraded = client.post("/api/v1/users/subscribe", json={"plan": "newsletter_pro"}, headers=headers)
    assert upgraded.status_code == 200
    assert upgraded.json()["plan"] == "newsletter_pro"
...
def test_archive_requires_newsletter_pro() -> None:
    token, _ = _register()
    headers = {"Authorization": f"Bearer {token}"}
    ...
    still_denied = client.get("/api/v1/newsletter/archive", headers=headers)
...
def test_newsletter_archive_links_to_research() -> None:
    token, _ = _register()
    headers = {"Authorization": f"Bearer {token}"}
    client.post("/api/v1/users/subscribe", json={"plan": "newsletter_pro"}, headers=headers)
...
def test_unsubscribe_keeps_research_access() -> None:
    ...
    assert status["tier"] == "newsletter_pro"
...
def test_checkout_newsletter_plan_is_forwarded() -> None:
    ...
    with patch(
        ...
        response = client.post("/api/checkout?plan=newsletter_pro", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert mocked.call_args.args[4] == "newsletter_pro"
